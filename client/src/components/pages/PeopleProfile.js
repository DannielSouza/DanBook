...
const PeopleProfile = () => {
  ...
  React.useEffect(()=>{
    async function getData(){
      await axios.get(`http://localhost:4000/user/profile/${id}`,{
      headers:{
  ...
  if(profile){
    if(profile.user[0].image) profilePicture = `url(http://localhost:4000/images/users/${profile.user[0].image})`
...
<main className={style.mainContainer}>
    ...
    {profile && profile.posts.map((item)=>{
      return <PostItem setAlert={setAlert} userId={profile.user[0]._id} key={item._id} date={item.createdAt} name={profile.user[0].name} picture={profile.user[0].image} postId={item._id} image={item.image} content={item.content} likes={item.likes} comments={item.comments}/>
    })}
    </div>
